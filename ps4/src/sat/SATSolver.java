 ...
 *
 * Algorithm DPLL
 * 		Input: A set of clauses ¦µ.
 *		Output: A Truth Value.
 * 	
 * function DPLL(¦µ)
 *  if ¦µ is a consistent set of literals
 *      then return true;
 *  if ¦µ contains an empty clause
 *      then return false;
 *  for every unit clause l in ¦µ
 *     ¦µ ¡û unit-propagate(l, ¦µ);
 *  for every literal l that occurs pure in ¦µ
 *     ¦µ ¡û pure-literal-assign(l, ¦µ);
 *  l ¡û choose-literal(¦µ);
 *  return DPLL(¦µ ¡Ä l) or DPLL(¦µ ¡Ä not(l));
 * */
